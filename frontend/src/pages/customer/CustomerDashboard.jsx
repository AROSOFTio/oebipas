...
import { CreditCard, Bell, Activity, Clock, AlertCircle } from 'lucide-react';
  ...
  const fetchSummary = async () => {
    try {
      ...
      if (err.response?.status === 404) {
        setError('No customer profile is linked to your account. Please contact an administrator.');
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-gradient-to-r from-sidebar to-blue-700 rounded-2xl shadow-md p-8 text-white flex flex-col justify-between">
           <div>
             ...
             {current_bill && (
               <div className="bg-white/10 rounded-lg p-3 inline-block backdrop-blur-sm">
                 <p className="text-sm"><span className="text-blue-200">Next due by:</span> <strong className="ml-1">{new Date(current_bill.due_date).toLocaleDateString()}</strong></p>
                 <p className="text-sm"><span className="text-blue-200">Current Bill:</span> <strong className="ml-1">{current_bill.bill_number}</strong></p>
               </div>
             )}
             {!current_bill && total_due == 0 && (
               <div className="bg-green-400/20 rounded-lg p-3 inline-block">
                 <p className="text-sm text-green-200">✓ Your account is up to date</p>
               </div>
             )}
           </div>
           <div className="mt-8 flex items-center space-x-4 flex-wrap gap-y-3">
             <Link to="/customer/pay" className="bg-white text-sidebar px-6 py-2.5 rounded-full font-bold shadow hover:bg-gray-50 flex items-center transition-transform hover:scale-105 active:scale-95">
               <CreditCard size={18} className="mr-2"/> Pay Now
             </Link>
             <Link to="/customer/bills" className="text-white hover:text-blue-100 font-medium underline-offset-4 hover:underline text-sm">
               View All Bills
             </Link>
           </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-6 flex flex-col">
           ...
           <div className="flex-1 space-y-3 overflow-y-auto pr-1 max-h-[220px]">
             {recent_notifications.map(n => (
               <div key={n.id} className={`relative pl-3 border-l-2 ${n.status === 'pending' ? 'border-primary' : 'border-gray-200'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
          <h2 className="font-bold text-gray-800 mb-6 flex items-center"><Activity size={18} className="mr-2 text-green-500"/> 6-Month Usage History (kWh)</h2>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={consumption_trend} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip cursor={{fill: '#F3F4F6'}} formatter={(val) => [`${val} kWh`, 'Consumption']} />
                  <Bar dataKey="units" fill="#10B981" radius={[4, 4, 0, 0]} barSize={40} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="h-64 flex items-center justify-center text-gray-400 text-sm">No consumption data yet.</div>
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-gray-800 flex items-center"><Clock size={18} className="mr-2 text-primary"/> Recent Payments</h2>
            <Link to="/customer/payments" className="text-xs text-primary font-medium hover:underline">View All</Link>
          </div>
          <div className="space-y-3">
             {recent_payments.map((p, i) => (
              ...
              {recent_payments.length === 0 && <p className="text-gray-400 text-sm text-center py-8">No payment history yet.</p>}
          </div>
        </div>
      </div>
    </div>
